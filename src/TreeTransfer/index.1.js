...
class TreeTransfer extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    const { treeNode, listData, leafKeys, expandedKeys } = this.generate(
      nextProps,
      this.state,
    );
    const treeCheckedKeys = listData.map(({ key }) => key);
    ...
    this.setState({
      ...
      leafKeys,
      treeCheckedKeys,
      treeExpandedKeys: searching
        ? uniq([...treeCheckedKeys, ...expandedKeys])
        : treeExpandedKeys,
    ...
    const loop = data =>
      data.map(item => {
        ...
        if (children === undefined) {
          ...
          return (
            <TreeNode key={key} title={nodeTitle} isLeaf {...otherProps} />
          );
        } else {
          // eslint-disable-line
          return (
  ...
  treeOnCheck = (checkedKeys, e) => {
    if (e.checked) {
      if (this.props.onLoadData && hasUnLoadNode([e.node])) {
        ...
        this.setState({
          treeCheckedKeys: checkedKeys.filter(
            key => this.state.leafKeys.indexOf(key) > -1,
          ), // eslint-disable-line
          unLoadAlert: false,
        });
      }
  ...
  listOnCheck = (e, checkedKeys) => {
    if (e.target.checked) {
      this.setState({
        listCheckedKeys: uniq([...this.state.listCheckedKeys, ...checkedKeys]), // eslint-disable-line
      });
    } else {
      this.setState({
        listCheckedKeys: this.state.listCheckedKeys.filter(
          key => checkedKeys.indexOf(key) < 0,
        ), // eslint-disable-line
      });
    }
  };
    ...
    this.setState(
      ...
      () => {
        if (this.props.onLoadData && this.props.onTreeSearch) {
          // eslint-disable-line
          this.props.onTreeSearch(value); // eslint-disable-line
        } else {
          const { treeNode, listData, leafKeys, expandedKeys } = this.generate(
            this.props,
            this.state,
  ...
  // render() {
  //   const { className, treeLoading, sourceTitle, targetTitle, showSearch, onLoadData } = this.props; // eslint-disable-line
  ...
  //         unLoadAlert: false
  //       });
  //       this.props.onChange && this.props.onChange(this.state.treeCheckedKeys); // eslint-disable-line
  //     }
  //   };
  ...
  //         unLoadAlert: false
  //       });
  //       this.props.onChange && this.props.onChange(this.state.listData.map(({key}) => key).filter(key => this.state.listCheckedKeys.indexOf(key) < 0));  // eslint-disable-line
  //     }
  //   };
...
TreeTransfer.propTypes = {
  ...
  onLoadData: PropTypes.func, // eslint-disable-line
  onTreeSearch: PropTypes.func, // eslint-disable-line
};
